...
class Live2DPet(QWidget):
    ...
    def _inject_js(self):
        """Ensure model loaded + hide UI."""
        js = """(function(){
  // If no model yet, try App.addModel directly
  if(window.App && window.App.addModel && !window.App.models.length) {
    try {
      ...
      console.log('[Senko] addModel fallback OK');
    } catch(e) {}
  }
        ...
        try:
            ...
            def do_voice():
                try:
                    ...
                    text = None
                # 用 QTimer 回到主线程显示对话框
                QTimer.singleShot(0, lambda: self._voice_done(text))
